...
module.exports.createFileData = (imageURL, publicId, userId, designTitle, designDescription) => {
    ...
    module.exports.getFileData = (userId, pageNumber, search) => {
        ...
        if ((search == '') || (search == null)) {
            ...
            designFileDataQuery = `SELECT file_id,cloudinary_url,design_title,design_description 
                FROM file  WHERE created_by_id=${userId} AND design_title LIKE '%${validator.blacklist(search,'\'')}%'  LIMIT ${limit} OFFSET ${offset};
                SET @total_records =(SELECT count(file_id) FROM file WHERE created_by_id= ${userId} AND design_title LIKE '%${validator.blacklist(search,'\'')}%' );SELECT @total_records total_records;`;
        }
        //--------------------------------------------------------------------
